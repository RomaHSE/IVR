...
interface SpendingsDataDBDao{
    ...
    @Query("SELECT s_amount FROM SpendingsDataDB WHERE s_type=:spType AND s_user=:uId")
    fun getByTypeAndUid(spType: String, uId: Int): List<Int>

    @Query("DELETE FROM SpendingsDataDB WHERE sid IN(:spendingsId)")
    fun deleteBySid(spendingsId: IntArray)

    @Query("DELETE FROM SpendingsDataDB WHERE s_user=:uId")
    fun deleteAllByUid(uId: Int)
